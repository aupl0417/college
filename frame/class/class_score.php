...
class score {
    ...
    static function getWhiteToRedTable() {
        ...
        $db->exec("SELECT a_uid,FLOOR(a_score / $returnUnionNumber) INTO OUTFILE '$dataFileName' FROM pay_account WHERE a_score >= $returnUnionNumber");
    ...
    static function getTodayStatistics($calcdate) {
        ...
        //累计个人会员、企业会员、联盟商家、开发者、代理公司、GP、LP
        $sql = "SELECT "
        ...
        $sql = "SELECT com_level, COUNT(*) as num FROM t_company WHERE com_createTime <= '$calcdate 23:59:59' GROUP BY com_level";
        $row = $dbErp->getAll($sql);
        foreach ($row as $v) {
        ...
        //今日个人会员、企业会员、联盟商家、开发者、代理公司、GP、LP
        $sql = "SELECT "
        ...
        while ($ret = $db->getAll("SELECT wrr_id,wrr_uid,wrr_returnUnit FROM pay_whitetored_return WHERE wrr_id % $num = $tid AND wrr_returnUnit > 0 AND wrr_isProgress = 0 LIMIT $step")) {
            ...
            try {
                ...
                foreach ($ret as $v) {
					...
					if(!$ac->transferScore(30201, $v['wrr_uid'], $nick, '', -1,  $score, '', 0, '',  $memo) || !$ac->transferTang(40101, $v['wrr_uid'], $nick, '', 1, $UnionScore, '', 0, '', $memo)){
