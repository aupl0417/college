...
class MySql {
    ...
    public function __construct($dbname = NULL) {
		$dbname = is_null($dbname) ? DB_NAME : $dbname;
        $this->setSource(DB_TYPE, DB_HOST, DB_PORT, DB_USER, DB_PASSWORD, $dbname, DB_PREFIX);
        $this->open();
    }
...
//    }
    ...
    //获得错误信息
    public function getError($e, $throwSql = true) {

        if ($e !== '') {
            
			$e .= ($throwSql) ? '  Sql:' . $this->lastSql() : '';
            echo $e;
            exit;
        }
    }
