...
class mgdb
{
    ...
    protected $_configData = array(
        'dsn'  =>  MONGO_DSN,
        'persist'   =>  true,
        'persist_key'   =>  'fdgdf$#@df5658F3^1239H',
        'replica_set'   =>  false,
        'query_safety'  =>  'safe'
    );
     ...
     */
    public $wheres = array();
    public $wheresBefore = array();//aggregate前置
    public $wheresAfter = array();//aggregate后筛选
	...
	/* 将要附加查询的字段添加进aggregate方法 */
	protected $_push2 = array();
	
	/* aggregate方法 Group Accumulator Operators */
	protected $_groupAcc = array();
    ...
    */
    ...
    public function where($wheres = array(), $value = null)
    {
        if (is_array($wheres)) {
            foreach ($wheres as $where => $value) {
                if(is_array($value)){
					if(isset($value['num']) && $value['num'] == '1'){
						$value = $value['value'] - 0;
					}else{
						$value = $value['value'];
					}
     ...
     * 
	 * $dataBetween = array(
		'field1' => array($min1, $max1),
		'field2' => array($min2, $max2),
	 );
     *
     * @access public
     * @return \MongoQB\Builder
     */
    public function whereBetween($dataBetween = array())
    {
        foreach($dataBetween as $field => $value){	
			$this->_whereInit($field);
			if(count($value) == 2){
				$this->wheres[$field]['$gte'] = $value[0];
				$this->wheres[$field]['$lte'] = $value[1];	
			}
		}
    ...
    */
	...
	/* 将要附加查询的字段添加进aggregate方法 */
	public function push2($array){
		if(!empty($array)){
			foreach($array as $alias => $field){
				$key = is_int($alias) ? $field : $alias;
				$this->_push2['$push'][$key] = '$'.$field;
			}
		}
		return $this;
	}
	...
	*/
	public function selectExt($fields){
		if(is_array($fields)){
	...
	* aggregate
	* 类似mysql group by方法
	* db.getCollection('queuelog').aggregate([
		{$match:{"result":0}},
		{$group:{_id:"$job", count:{$sum:1}, params:{$push:"$param"}, waits:{$push:"$wait"}, results:{$push:"$result"}, levels:{$push:"$level"}, times:{$push:"$time"}}},
		{$sort:{"count":-1}},
		{$limit:11},
		{$skip:1}
		]);	
	*/
	...
	public function aggregate($fields, $collection){
        if (empty($fields)) {
		...
		$maxSearch = 10000;//最大查询数量,超过该数值不翻页
		$cache = new cache();
		$all[] = $group;//group by总数
		$allCacheKey = md5(json_encode($all));
		$total = $cache->get($allCacheKey);
		if(!$total){
			$allRs = $this->_dbhandle->{$collection}->aggregate($all);
			if($allRs['ok']){
				$total = count($allRs['result']);
				if($total >= $maxSearch){
					$cache->set($allCacheKey, $maxSearch, 86400*365);
				}
			}else{
				return false;
			}
		...
		/* 每一项总数 */
		$group['$group']['count'] = array('$sum' => 1);
		
		if(!empty($this->_groupAcc)){
			$group['$group'] = array_merge($group['$group'], $this->_groupAcc);
		}
	
		/* 附加选择字段 */
		if(!empty($this->_push2)){
			$group['$group']['pushData'] = $this->_push2;
		}else{//如果没有指定附加字段,那么返回整条记录
			$group['$group']['pushData'] = ['$addToSet' => '$$ROOT'];
		}
		
		$pipeline[] = $group;
		$filter = $pipeline;
		/* 前置检索条件 */
		$matchBefore = $this->wheresBefore;
		if(!empty($matchBefore)){			
			array_unshift($pipeline, ['$match' => $matchBefore]);
			//$pipeline[] = ['$match' => $match];
			array_unshift($filter, ['$match' => $matchBefore]);
			//$filter[] = ['$match' => $match];
		}
		array_unshift($pipeline, ['$sort' => ['_id' => 1]]);
		/* 后筛选检索条件 */
		$matchAfter = $this->wheresAfter;
		if(!empty($matchAfter)){			
			$pipeline[] = ['$match' => $matchAfter];
			$filter[] = ['$match' => $matchAfter];
		}
		
		/* 符合检索条件的记录总数 */		
		$filterCacheKey = md5(json_encode($filter));
		$totalFilter = $cache->get($filterCacheKey);
		if(!$totalFilter){
			$filterRs = $this->_dbhandle->{$collection}->aggregate($filter);
			if($filterRs['ok']){
				$totalFilter = count($filterRs['result']);
				if($totalFilter >= $maxSearch){
					$cache->set($filterCacheKey, $maxSearch, 86400*365);
				}
			}else{
				return false;
			}
		...
		/* $project */
		if(!empty($project)){
			array_unshift($pipeline, ['$project' => $project]);
		}
	 ...
	 * array(
	 *	'count' => array('sum'=>1),
	 *	'lastTime' => array('last'=>'$time'), 
	 * ); 
	 * 
	 * */
	public function groupAcc($acc){
        if (empty($acc)) {
            throw new Exception('字段不存在');
        }
		foreach($acc as $alias => $fields){
			foreach($fields as $type => $field){	
				$this->_groupAcc[$alias] = [
					'$'.$type => $field
				];
			}
		}
