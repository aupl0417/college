...
class classListGet_json extends api {
    function run() {
        ...
        $db = new MySql();
        $sql = 'SELECT cl.*,br_name FROM tang_class cl LEFT JOIN tang_branch br ON br.br_id=cl.tangCollege WHERE cl_state IN(0,1) AND cl_status=1 ORDER BY cl_createTime DESC';
        ...
        $sql = "SELECT tse_classId,count(1) total FROM tang_student_enroll WHERE tse_classId IN($classIdList) AND tse_state IN(0,1) GROUP BY tse_classId";
