...
// 		dump($options);die;
		...
		$charName = isset($options['charName'])? $options['charName']: "";
		if($name == "" ){//
...
// 		}	
		...
		if ($department){
    		...
    		$update['e_departmentID'] = $dep['dm_id'];
		}
		//print_r($update);
		$db = new MySql();		
		...
		//记录操作日志
		$update['memo'] = '雇员资料修改';//尽量写得详细一点点了
		log::writeLogMongo(20202, 't_employee', $id, $update);
		if($result){			
