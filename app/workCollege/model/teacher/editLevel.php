...
class editLevel extends worker {
    ...
    function run() {
		...
		$teacherLevel['courseLevel'] = $this->getSelectString($courseLevel, $teacherLevel['tl_courseLevel']);
		$teacherLevel['courseType'] = $this->getSelectString($courseType, $teacherLevel['tl_courseType']);
    ...
    private function getSelectString($data, $keyValue){
        if(!is_array($data)){
            return;
        }
