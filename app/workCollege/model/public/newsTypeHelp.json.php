...
/* 
	通过资讯平台编码取得下级联动
 */
class newsTypeHelp_json extends worker {
    ...
    function run() {
		...
		$db = new MySql();
		if($id == 0){//取出所有一级分类
			$sql = "select nt_code as code, nt_typename as `name`, nt_id as id, SUBSTR(nt_code, 1, 2) as `key` from t_news_type where nt_id=4";
