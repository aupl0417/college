...
class divPages
{
    ...
    public function __construct($table, $fields = '*', $pagenow = 1, $pagesize = 10, $menustyle = 1, $where = '', $join = '', $order = '')
    {
    ...
    public function getDivPage()
    {
        ...
        $sql2 = "SELECT $this->fields FROM {$this->table} {$this->join} {$this->wherestr} {$this->orderstr} limit $_beginRow,$this->pagesize";
//         echo $sql2;die;
        $db = new MySql();
        //得到页数据
    ...
    public function setMenuStyle($ms = 1)
    {
        ...
        switch ($this->menustyle) {
            ...
            1 :
                ...
                $this->pagemenu .= "<li> <a href=\"javascript:go({fpage})\" target=\"_self\"> 上一页 </a> </li>";
                for ($i = 1; $i <= $this->pageinfo['pagecount']; $i++) {
                    if ($this->pagenow == $i){
                        $this->pagemenu .= "<li class=\"active\"> <a href=\"javascript:go($i);\" target=\"_self\"> $i </a> </li>";
                    }else{
                        $this->pagemenu .= "<li> <a href=\"javascript:go($i);\" target=\"_self\"> $i </a> </li>";
                    }
                }
