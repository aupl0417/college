...
class index extends baseApi{
	...
	function run(){
		...
		(empty($userId) || empty($periodId)) && $this->apiReturn(200, '参数不能为空'); //用户id和课时id必填
        ...
        /*某班级今天上课的所有课程*/
		$sql = "SELECT cta_classId,cta_startTime,cta_endTime FROM tang_class_table
            WHERE cta_startTime BETWEEN '".$attendStatTime."' AND '$attendEndTime' AND cta_classId='{$classID}' ORDER BY cta_startTime ASC";
		$periodData = $this->db->getAll($sql);

		empty($periodData) && $this->apiReturn(202, '今天没有课程！');
		
		//将用户加入该课时所对应的班级
		$q = $this->db->getField("select count(cs_id) as count from tang_class_student where cs_classId='" .$classID. "' and cs_studentId='" . $uid . "'");
		if (!$q) {
		    $this->apiReturn(210, '用户不在该班级');
		}

        $firstCourse = pos($periodData);
        $lastCourse = array_pop($periodData);
        
		$startTime = strtotime($firstCourse['cta_startTime']);
		$endTime   = strtotime($lastCourse['cta_endTime']);
