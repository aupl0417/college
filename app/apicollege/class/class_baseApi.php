...
class baseApi{
	...
	protected $app = array(
         ...
         '3' => 'D71706B84122BC48D715BAD075065F2A', //web
	);
	protected $db;
    ...
    function __construct($options) {
		...
		/*$keysArray = array('PATH_INFO', 'PATH_MODEL', 'PATH_ACTION', 'SS_MID', 'PHPSESSID', 'Files','sign', '__utma', '__utmz');
...
// 		    if($options['PATH_INFO'] != '/Enroll/index.json' || $options['PATH_INFO'] != '/Enroll/index'){//如果是报名接口，则同步数据，否则，不同步用户数据
// 		        $params['sync'] = 0;
// 		    }
		    
            $result = apis::request('/college/api/getUser.json', $params, true);
            
		    !is_array($result) && $this->apiReturn(1002, $result);
		    ($result['code'] != '1001') && $this->apiReturn(1002, $result['data']);
		    empty($result['data']) && $this->apiReturn(201);
	...
	protected function signValidate($data, $sign){
        ...
        //echo md5("{$queryString}&{$secretKey}").'和'.$sign ; exit;
