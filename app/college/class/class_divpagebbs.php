 ...
 */
//交易记录扩展分页类
class divBbsPage extends divpage
{
    ...
    public function getDivPage()
    {
        ...
        $sql1 = "SELECT count(*) FROM t_bbs $this->wherestr";
        $sql2 = "SELECT $this->fields FROM t_bbs $this->wherestr order by b_isTop desc, b_createTime desc limit $_beginRow,$this->pagesize";
        $db = new MySql();
        //得到页数据
    ...
    public function setMenuStyle($ms = 1)
    {
        ...
        switch ($this->menustyle) {
            ...
            2 :
                $this->pagemenu .= "<replace value=\"total|pagenow|fpage|npage|lastpage|pagecount|pagesize\">";
                $this->pagemenu .= "<div class=\"bbs-paging over mb20\">";
                $this->pagemenu .= "<ul class=\"list-unstyled pull-left ml20\">";
                $this->pagemenu .= "<li> <a href=\"javascript:go({fpage},search)\" class=\"perv\"> 上一页 </a> </li>";
                for ($i = 1; $i <= $this->pageinfo['pagecount']; $i++) {
                    if ($this->pagenow == $i){
                        $this->pagemenu .= "<li class=\"active\"> <a href=\"javascript:go($i,search);\"> $i </a> </li>";
                    }else{
                        $this->pagemenu .= "<li> <a href=\"javascript:go($i,search);\"> $i </a> </li>";
                    }
                }
                $this->pagemenu .= "<li> <a href=\"javascript:go({npage},search)\" class=\"next\"> 下一页 </a> </li>";
                $this->pagemenu .= "</ul>";
                $this->pagemenu .= "<a href=\"/bbs/posted\" class=\"pull-right bbs-posting-btn mr20\">发帖</a>";
                $this->pagemenu .= "</div>";
                $this->pagemenu .= "</replace>";
                break;
            ...
            3 :
                $this->pagemenu .= "<replace value=\"total|pagenow|fpage|npage|lastpage|pagecount|pagesize\">";
                $this->pagemenu .= "<div class=\"bbs-paging over mt35\">";
                $this->pagemenu .= "<ul class=\"list-unstyled pull-left ml20\">";
                $this->pagemenu .= "<li> <a href=\"javascript:go({fpage},search)\" class=\"perv\"> 上一页 </a> </li>";
                for ($i = 1; $i <= $this->pageinfo['pagecount']; $i++) {
                    if ($this->pagenow == $i){
