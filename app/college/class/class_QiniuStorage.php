...
class QiniuStorage {
	...
	const ACCESSKEY    = 'kHuoywSGq_16RchDS9bXQgWXTlHNCG5XnOF3tJba';
	const SECRETKEY    = 'KqovkLXhX1-NyqQCKVBmJgOj0dfsYHoJ7hRtn1hM';
	public $BUCKET          = 'aupl0401'; //空间名称
	public $DOMAIN          = 'oimslmq8z.bkt.clouddn.com';
	private $PIPELINE       = 'avvod-pipeline';//队列名称
	private $NOTIFYURL      = 'http://workcollege.dterptest.com/public/notify';  //回调地址
	public $timeout = '';
	public static $auth;
    
	public function __construct(){
		$this->sk = self::SECRETKEY;
		$this->ak = self::ACCESSKEY;
	...
	static function getAuth(){
		return new QiniuAuth(self::ACCESSKEY, self::SECRETKEY);
	}
	...
	public function execute($key, $fops)
	{
	    ...
	    $this->setWithoutEmpty($params, 'pipeline', $this->PIPELINE);
	    $this->setWithoutEmpty($params, 'notifyURL', $this->NOTIFYURL);//$this->NOTIFYURL
	    if ($this->force) {
	        $params['force'] = 1;
	    }
	    $data = http_build_query($params);
	    $url = $this->QINIU_API_HOST . '/pfop/';
	    $auth = new QiniuAuth(self::ACCESSKEY, self::SECRETKEY);
	    $headers = $auth->authorization($url, $data, 'application/x-www-form-urlencoded');
	...
	{
	    $url = $this->QINIU_API_HOST . "/status/get/prefop?id=$id";
