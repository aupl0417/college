                    ...
                    foreach ( $data[ 'targets' ] as $target ) {
                        ...
                        switch ( $targetArr[ 0 ] ) {//0-全部;1-全部会员;2-全部雇员;3-指定会员;4-指定雇员;//Android Token长度为40位,iOS Token长度为64位
                            case 0:
								...
								$appNotice->employee($data[ 'title' ]);//发送workapp推送
                                break;
                            case 1:
                                switch ( $targetArr[ 1 ] ) {
                                    case 0: //全部会员
                            ...
                            case 2:
                                ...
                                break;
                            case 3: //3-指定会员;
                                $token = self::$db->getField( "SELECT u_token FROM t_app_xinge WHERE u_id='{$targetArr[1]}' AND u_status = 1 LIMIT 1" );
                                if($token){
                                    (strlen( $token ) == 40) ? Xinge::PushTokenAndroid( $data[ 'title' ], $pushContent, $token ) : Xinge::PushTokenIos( $pushContent, $token, $iosenv );
                                }
                                break;
                            case 4: //4-指定雇员
								
								$appNotice->employee($data[ 'title' ], [$targetArr[1]]);//发送workapp推送
