    ...
    e.AmRectangularChart = e.Class({
        ...
        drawZoomOutButton: function() {
            ...
            if (!a.zbSet) {
                ...
                c = 1;
                e.isModern || (c = 0);
                k = e.rect(a.container, n.width + 2 * q + 5, n.height + 2 * q - 2, k, 1, 1, k, c);
                k.setAttr("opacity", l);
                k.translate(-q, -q);
    ...
    e.AmSerialChart = e.Class({
        ...
        updateChartCursor: function() {
            ...
            if (a) {
                ...
                if (a.valueLineBalloonEnabled) for (k = 0; k < this.valueAxes.length; k++) h = this.valueAxes[k], (f = h.balloon) || (f = {}), f = e.extend(f, this.balloon, !0), f.fillColor = d, f.balloonColor = d, f.fillAlpha = c, f.borderColor = d, f.color = g, h.balloon = f;
                else for (f = 0; f < this.valueAxes.length; f++) h = this.valueAxes[f], h.balloon && (h.balloon = null);
                b && (b.balloonTextFunction = a.categoryBalloonFunction, a.categoryLineAxis = b, b.balloonText = a.categoryBalloonText, a.categoryBalloonEnabled && ((f = b.balloon) || (f = {}), f = e.extend(f, this.balloon, !0), f.fillColor = d, f.balloonColor = d, f.fillAlpha = c, f.borderColor = d, f.color = g, b.balloon = f))
            }
        },
    ...
    e.CategoryAxis = e.Class({
        ...
        draw: function() {
            ...
            if (e.ifArray(a)) {
                ...
                if (this.parseDates && !this.equalSpacing) this.lastTime = a[a.length - 1].time, this.maxTime = e.resetDateToMin(new Date(this.lastTime + 1.05 * x), this.minPeriod, 1, q).getTime(), this.timeDifference = this.endTime - this.startTime, this.parseDatesDraw();
                    ...
                    if (this.cellWidth = this.getStepWidth(f), f < k && (k = f), h += this.start, this.stepWidth = this.getStepWidth(f), 0 < k) for (k = Math.floor(f / k), f = this.chooseMinorFrequency(k), p = h, p / 2 == Math.round(p / 2) && p--, 0 > p && (p = 0), v = 0, this.widthField && (p = this.start), this.end - p + 1 >= this.autoRotateCount && (this.labelRotationR = this.autoRotateAngle), b = p; b <= this.end + 2; b++) {
                        ...
                        if (t && !isNaN(f)) if (b / f == Math.round(b / f) || n) b / k == Math.round(b / k) || n || (this.gridAlpha = this.minorGridAlpha, p = void 0);
                        else continue;
