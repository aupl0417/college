	...
	ace.demo.functions.enableDemoAjax = function() {
		...
		var dttx_ajax_options = {
			 ...
			 'content_url': function(hash) {
				...
				//console.log(document.location, origin, 'sdfsdf', hash);
				path = origin + hash;
		...
		//if(window.Pace) {
			dttx_ajax_options['loading_overlay'] = 'body';//the opaque overlay is applied to 'body'
		//}

		//initiate ajax loading on this element( which is .page-content-area[data-ajax-content=true] in Ace's demo)
		$('[data-ajax-content=true]').ace_ajax(dttx_ajax_options)
	...
	function AceAjax(contentArea, settings) {
		...
		this.getUrl = function(url, hash, manual_trigger, method, data, updateView, extraParams) {
			...
			.done(function(result) {
				...
				if (/^\{\"id\"\:\"\d+\"\,\"msg\"[\s\S]+?\}$/.test(result)) {//返回的其实是一段json字符串,一般是权限问题
					...
					if (res.info && typeof res.info == 'object') {
						if (res.info.text) {
							bootbox.alert(res.info.text, function () {
								//Global.unblockUI();
								if (res.info.url) {
									if(/http[s]?\:\/\/.+/ig.test(res.info.url)){
										document.location.href = res.info.url;
									}else{
										document.location.hash = res.info.url;
									}
								}
							});
						}
				...
				if(updateView) {
					...
					result = String(result)
						...
						.replace(/(\<a.+?href\=\")((?!#|http|javascript|\{\{).+)(\")/gi, '$1#$2$3')
					
					$contentArea.empty().html(result);
					/* 重新绑定页面上的脚本 */
					if(Global){
						Global.initAjax($contentArea);
					}
				}
				
				init_Menu_Breadcrumbs_Message_Info(currentMenuId);
				var link_text = page_setting.title;
		...
		this.startLoading = function() {
			...
			if(typeof layer != 'undefined'){
				...
				$overlay.remove();
				$overlay = $('<div class="ajax-loading-overlay text-center" style="padding-top:200px;"><i class="ajax-loading-icon '+(this.settings.loading_icon || '')+'"></i> '+this.settings.loading_text+'</div>')
		...
		this.scriptsObject = {
			'fancybox': {
				'obj':'jQuery.fancybox',
				'path':'/app/public/assets/plugins/fancybox/source/jquery.fancybox.pack.js',
			},
				...
				for(var i = 0; i < scripts.length; i++){ 					
					if(scripts[i]) {
						if(scripts[i].indexOf('.') == -1){							
							...
							if(self.scriptsObject[scripts[i]]){
								if(!eval(self.scriptsObject[scripts[i]].obj)){
									scripts[i] = self.scriptsObject[scripts[i]]['path'];	
								}else{
									continue;
								};
							}else{
		...
		if(!hash && (pathname == '/' || pathname == '/index' || pathname == '/index/index')){
		...
		if( navbar.css('position') == 'fixed' ) {
			...
			if( breadcrumbs.length > 0 ) {				
				if(breadcrumbs.css('position') == 'fixed') {
					...
					//console.log(default_padding, breadcrumbs.outerHeight(), parseInt(breadcrumbs.css('margin-top')));
