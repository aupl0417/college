...
/* 		jQuery.validator.addMethod("datetime", function(value, element) {
		...
		jQuery.validator.addMethod("isusername", function(value, element) {
				return this.optional(element) || (/^[a-zA-Z\u4e00-\u9fa5]{1}[\u2027·a-zA-Z0-9\_\u4e00-\u9fa5]{4,48}[a-zA-Z0-9\u4e00-\u9fa5]{1}$/.test(value) && !/^(ylh|yunlian|云联|客服)/i.test(value) && !/^(tel|pos)(13[0-9]|15[012356789]|17[0678]|18[0-9]|14[57])[0-9]{8}$/i.test(value));
			}, 
		"用户名格式错误");
		...
		jQuery.validator.addMethod("islicense", function(value, element) {
				return this.optional(element) || /^[\w\-]{6,}/i.test(value);
			}, 
		"营业执照格式错误");
		...
		form.$el.validate({
			...
			errorPlacement: function (error, element) { // 处理错误信息
				...
				//console.log(el.indexOf('['));return false;
...
/* 				if($('#'+ el +'-error').length > 0){
		...
		var submitForm = function(subForm, form, e){
			//console.log(subForm, form, e);return false;
