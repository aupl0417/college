...
(jQuery), jQuery(function (a) {
	function b() {
		...
		ace.vars.transition = !!a.support.transition.end
	}
	function c() {
		var b = a(".sidebar");
...
(ace.helper), function (a, b) {
	function c(b, c) {
		...
		k = a();
		this.force_reload = !1,
		this.loadUrl = function (a, b) {
			var c = !1;
			a = a.replace(/^(\#\!)?\#/, ""),
			this.force_reload = b === !1,
			"function" == typeof this.settings.content_url && (c = this.settings.content_url(a)),
			"string" == typeof c && this.getUrl(c, a, !1)
		},
						...
						if ("function" == typeof h.settings.update_active)
							...
							if (m.length > 0) {
								...
								m.closest(".sidebar[data-sidebar-scroll=true]").each(function () {
									...
									d && b.ace_sidebar_scroll("scroll_to_active")
								})
							}
						}
						"function" == typeof h.settings.update_breadcrumbs ? l = h.settings.update_breadcrumbs.call(null, c, b, j) : h.settings.update_breadcrumbs === !0 && null != j && j.length > 0 && (l = e(j)),
						i = String(i).replace(/<(title|link)([\s\>])/gi, '<div class="hidden ajax-append-$1"$2').replace(/<\/(title|link)\>/gi, "</div>"),
						...
						setTimeout(function () {
							...
							g.find(".ajax-append-link").each(function () {
								...
								if (b.attr("href")) {
									...
									c.length > 0 ? d.insertBefore(c) : d.appendTo("head"),
									d.attr("href", b.attr("href"))
								}
								b.remove()
							})
						}, 10),
						"function" == typeof h.settings.update_title ? h.settings.update_title.call(null, c, b, l) : h.settings.update_title === !0 && f(l),
	...
	a.fn.aceAjax = a.fn.ace_ajax = function (d, e, f, g) {
		...
		i = this.each(function () {
				...
				j = i.data("ace_ajax"),
				k = "object" == typeof d && d;
				j || i.data("ace_ajax", j = new c(this, k)),
				"string" == typeof d && "function" == typeof j[d] && (h = g != b ? j[d](e, f, g) : f != b ? j[d](e, f) : j[d](e))
			});
		return h === b ? i : h
	},
...
(window.jQuery), function (a, b) {
	function c(c, e) {
		function f() {
			...
			this.minimized = !this.collapsible && this.$sidebar.hasClass(l) || 3 == this.mobile_style && this.mobile_view && this.$sidebar.hasClass(m),
		...
		j = function () {
			...
			return 0 == b.length && (b = a('.sidebar-collapse[data-target="#' + (g.$sidebar.attr("id") || "") + '"]')),
		...
		this.toggleMenu = function (a, b) {
			if (!this.collapsible) {
				...
				a || (a = j()),
				a && i.call(a, this.minimized),
				ace.vars.old_ie && ace.helper.redraw(c)
			}
		},
		...
		this.$sidebar.on(ace.click_event + ".ace.submenu", ".nav-list", function (b) {
			...
			if (d && 0 != d.length) {
				...
				if (d.hasClass("dropdown-toggle")) {
					...
					k ? (g.show(f, g.settings.duration), 0 != i && (i += f.scrollHeight)) : (g.hide(f, g.settings.duration), i -= f.scrollHeight),
					0 != i && ("true" != g.$sidebar.attr("data-sidebar-scroll") || g.minimized || g.$sidebar.ace_sidebar_scroll("prehide", i)),
					!1
				}
		...
		this.show = function (b, c, d) {
			...
			return c > 0 && a.support.transition.end ? f.one(a.support.transition.end, g) : g(),
		...
		o = function () {
			this.mobile_style = 1,
			this.$sidebar.hasClass("responsive") && !a('.menu-toggler[data-target="#' + this.$sidebar.attr("id") + '"]').hasClass("navbar-toggle") ? this.mobile_style = 2 : this.$sidebar.hasClass(m) ? this.mobile_style = 3 : this.$sidebar.hasClass("navbar-collapse") && (this.mobile_style = 4)
		};
		o.call(g),
	...
	a(document).on(ace.click_event + ".ace.menu", ".menu-toggler", function (b) {
		...
		if (0 != d.length) {
			b.preventDefault(),
			d.toggleClass("display"),
			c.toggleClass("display");
	...
	a.fn.ace_sidebar = function (d, e) {
		...
		g = this.each(function () {
				...
				h = "object" == typeof d && d;
				g || b.data("ace_sidebar", g = new c(this, h)),
	...
	function c(b, c) {
		...
		if (l) {
			...
			y = function () {
				...
				return f.sidebar_fixed && (a.top -= ace.helper.scrollTop()),
				g.innerHeight() - a.top - (f.settings.include_toggle ? 0 : j.outerHeight())
			},
			z = function () {
			...
			A = function (b) {
				if (!v && f.sidebar_fixed) {
					i.wrap('<div class="nav-wrap-up pos-rel" />'),
					i.after("<div><div></div></div>"),
					i.wrap('<div class="nav-wrap" />'),
					f.settings.include_toggle || j.css({
						"z-index" : 1
					}),
					...
					f.settings.include_shortcuts && 0 != k.length && (i.parent().prepend(k).wrapInner("<div />"), i = i.parent()),
					f.settings.include_toggle && 0 != j.length && (i.append(j), i.closest(".nav-wrap").addClass("nav-wrap-t")),
					i.css({
						position : "relative"
					}),
					1 == f.settings.scroll_outside && p.addClass("scrollout"),
					l = i.get(0),
					l.style.top = 0,
					...
					if (i.on("ace_drag.nav", function (b) {
							if (!f.is_scrolling || !u.is_active())
								...
								if ("up" == b.direction || "down" == b.direction) {
									...
									Math.abs(d) > 2 && (d = 2 * d),
									0 != d && (c.scrollTop = c.scrollTop + d, l.style.top = -1 * c.scrollTop + "px")
								}
							}), f.settings.smooth_scroll && i.on("touchstart.nav MSPointerDown.nav pointerdown.nav", function () {
								i.css("transition-property", "none"),
			...
			this.reset = function (a) {
				...
				d && u.is_active() ? h.addClass("sidebar-scroll") : this.is_scrolling && this.disable()
			},
			this.disable = function () {
				...
				parseInt(l.style.top) < 0 && f.settings.smooth_scroll && a.support.transition.end ? i.one(a.support.transition.end, function () {
					h.removeClass("sidebar-scroll"),
					i.off(".trans")
				}) : h.removeClass("sidebar-scroll"),
				l.style.top = 0
			},
			...
			h.on("hidden.ace.submenu.sidebar_scroll shown.ace.submenu.sidebar_scroll", ".submenu", function (a) {
				a.stopPropagation(),
				o.get("minimized") || (f._reset(), "shown" == a.type && f.set_hover())
			}),
			A(!0)
		}
	}
...
(window.jQuery), function (a, b) {
	function c(b, c) {
		...
		function k(c) {
			...
			B = q.horizontal,
			C = !1;
			B && this.parentNode == p && (x = 0, m.top += d.height(), C = !0),
			!C && (x = u - (y + h)) >= 0 && (x = z > x ? x : z, 0 == x && (x = 20), z - x > 10 && (x += parseInt(Math.min(25, z - x))), m.top + (n - o) > u - x && (x -= m.top + (n - o) - (u - x)), x > 0 && (c.style.top = -x + "px", g && (g.style.marginTop = -x + "px"))),
			0 > x && (x = 0);
			var D = x > 0 && x > n - 20;
		...
		o.on(y, ".nav-list li, .sidebar-shortcuts", function () {
			...
			q.collapsible || a(this).hasClass("hover-show") && i(this).hideDelay()
		})
	}
...
(window.jQuery), function (a, b) {
	...
	var d = function (b) {
		...
		this.fullscreen = function () {
			...
			b = null,
			d = null;
			(b = a.attr("data-icon1")) ? d = a.attr("data-icon2") : (b = "fa-expand", d = "fa-compress"),
			this.$box.hasClass("fullscreen") ? (a.addClass(b).removeClass(d), this.$box.removeClass("fullscreen"), c(this.$box, !1)) : (a.removeClass(b).addClass(d), this.$box.addClass("fullscreen"), c(this.$box, !0)),
